...
echo "<pre>Setup by sellervps | sellervps.net | @sellervps | 7946F434</pre>" > /home/vps/public_html/index.html
...
sed -i 's/DROPBEAR_EXTRA_ARGS=/DROPBEAR_EXTRA_ARGS="-p 443 -p 110 -p 80"/g' /etc/default/dropbear
...
mkdir bash
cd /root/bash
wget "https://raw.githubusercontent.com/aliya02/centos/master/bash.sh.zip"
unzip bash.sh.zip
...
chmod +x squid3
chmod +x webmin
chmod +x rebootserver
chmod +x autoexpired.sh
chmod +x bannerlogin
chmod +x cacheram
chmod +x password
chmod +x portopenssh
...
cd
cd /user/bin
wget "https://raw.githubusercontent.com/aliya02/centos/master/menu"
chmod +x menu
...
# info
clear
echo "seller-vps.net | @DYP| DYP | 7946F434" | tee log-install.txt
...
echo "---------------"  | tee -a log-install.txt
echo "User     : sellervps"  | tee -a log-install.txt
